...
class PrivacyPolicy_WV0 extends StatefulWidget {
  @override
  _PrivacyPolicy_WV0State createState() => _PrivacyPolicy_WV0State();
}

class _PrivacyPolicy_WV0State extends State<PrivacyPolicy_WV0> {
    ...
    Widget w_webview0 = WebView(
      ...
      onPageFinished: (String url) {
        ...
        if(idf_user!="" && ulang1==0){
          _webViewController.loadUrl(AppConfig().url_APIRoot+'mobile/mob_privacy_policy.php?idf_user='+idf_user);
          debugPrint("lagiiiii");
          setState(() {
            ulang1 += 1;
          });
          ...
          NavigationDelegate(
            ...
            onPageFinished: (String url) {
              ...
              if (idf_user != "" && ulang1 == 0) {
                ...
                String newUrl = AppConfig().url_APIRoot + 'mobile/mob_privacy_policy.php?idf_user=' + idf_user;
                _webViewController.loadRequest(Uri.parse(newUrl)); // Load the URL after initial setup
              }
            },
            onWebResourceError: (WebResourceError error) {
              print('Page error loading: $error');
              setState(() {
                noInternetVis = true; // Handle no internet situation
              });
            },
          ),
        )
  ...
  refreshUlang() {
    ...
    if(idf_user!="" && ulang1==0){
      //_webViewController.loadUrl(AppConfig().url_APIRoot+'mobile/mob_privacy_policy.php?idf_user='+idf_user);
      _webViewController.loadRequest(Uri.parse(AppConfig().url_APIRoot+'mobile/mob_privacy_policy.php?idf_user='+idf_user));
      debugPrint("lagiiiii");
      setState(() {
        ulang1 += 1;
      });
